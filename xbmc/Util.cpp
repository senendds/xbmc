...
void CUtil::ScanForExternalSubtitles(const std::string& strMovie, std::vector<std::string>& vecSubtitles)
{
  ...
  CLog::Log(
      ...
      "isinternet={} mimetype='{}'",
      item.GetPath(), item.GetDynPath(), isStrmAnchor, item.IsPlayList(), item.IsVideo(),
      item.IsInternetStream(), item.GetMimeType());
  if ((item.IsInternetStream() && !URIUtils::IsOnLAN(item.GetDynPath()))
    || (!isStrmAnchor && item.IsPlayList())
    || item.IsLiveTV()
    || (!isStrmAnchor && !item.IsVideo()))
    return;
